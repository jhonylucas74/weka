...
/** import all split criteria in the package */
 ...
 * <pre> -C &lt; mi | ami | gmi &gt;
 *  Splitting criterion.
 *  (Default = Mutual Information or Information Gain as Weka Random
 *  Forests)
 * </pre>  
 * 
 ...
 */
public class RandomTreeSplitCriterionSelection extends AbstractClassifier implements OptionHandler,
WeightedInstancesHandler, Randomizable, Drawable {

  /** for serialization */
  static final long serialVersionUID = 8934314652175299374L;
  
  /** default is Mutual Information */
  public static final int SPLITCRIT_DEFAULT = 1;
  public static final int SPLITCRIT_AMI = 2;
  public static final int SPLITCRIT_GMI = 3;
  
  public static final Tag [] TAGS_SPLITCRIT = {
    new Tag(SPLITCRIT_DEFAULT, "Default, MI as Standard Random Forests"),
    new Tag(SPLITCRIT_AMI, "Adjusted Mutual Information"),
    new Tag(SPLITCRIT_GMI, "Grassberger Mutual Information")
  };
  
  /** Splitting criterion used */
  protected int m_splitCrit = SPLITCRIT_DEFAULT;
  
  /** The subtrees appended to this tree. */
  protected RandomTreeSplitCriterionSelection[] m_Successors;
   ...
   */
  public String splitCritTipText() {
    return "The splitting criterion used to split nodes.";
  }

  /**
   * Gets the currently set splitting criterion
   * 
   * @return the splitting criterion used
   */
  public SelectedTag getSplitCrit() {
    return new SelectedTag(m_splitCrit, TAGS_SPLITCRIT);
  }

  /**
   * Sets the splitting criterion
   * 
   * @param splitCrit the new splitting criterion to be used
   */
  public void setSplitCrit(SelectedTag splitCrit) {
    if (splitCrit.getTags() == TAGS_SPLITCRIT) {
      m_splitCrit = splitCrit.getSelectedTag().getID();
    }
  }
  ...
  public Enumeration listOptions() {
    ...
    newVector.addElement(new Option(
			"\tSplitting criterion.\n" +
			"\t(Default = Mutual Information or Information Gain as Weka Random Forests))",
			"C", 1, "-C <mi | ami | gmi >"));
  ...
  public String[] getOptions() {
    ...
    if (m_splitCrit != SPLITCRIT_DEFAULT) {
      result.add("-C");
      switch (m_splitCrit) {
			case SPLITCRIT_AMI:
	      result.add("ami");
  ...
  public void setOptions(String[] options) throws Exception {
    ...
    if (tmpStr.length() != 0) {
      if (tmpStr.equals("mi")) {
				setSplitCrit(new SelectedTag(SPLITCRIT_DEFAULT, TAGS_SPLITCRIT));
			} else if (tmpStr.equals("ami")) {
				setSplitCrit(new SelectedTag(SPLITCRIT_AMI, TAGS_SPLITCRIT));
			} else if (tmpStr.equals("gmi")) {
				setSplitCrit(new SelectedTag(SPLITCRIT_GMI, TAGS_SPLITCRIT));
			} else {
				throw new IllegalArgumentException("Invalid splitting criterion");
			}
		}
  ...
  protected void buildTree(Instances data, double[] classProbs, Instances header,
    ...
    while ((windowSize > 0) && (k-- > 0 || !gainFound)) {
      ...
      // get the criterion chosen 
      Split s = getCriterion(dists[attIndex]);
      vals[attIndex] = s.gain();
    ...
    if (Utils.gr(vals[m_Attribute], 0)) {
      ...
      Instances[] subsets = splitData(data);
      m_Successors = new RandomTreeSplitCriterionSelection[distribution.length];
      for (int i = 0; i < distribution.length; i++) {
        m_Successors[i] = new RandomTreeSplitCriterionSelection();
        m_Successors[i].setKValue(m_KValue);
        m_Successors[i].setSplitCrit(new SelectedTag(m_splitCrit, TAGS_SPLITCRIT));
  ...
  throws Exception {
    ...
    if (attribute.isNominal()) {
      ...
      // Shift the first instance, no useful split there
      Instance inst = data.instance(0);
      currDist[0][(int) inst.classValue()] += inst.weight();
      currDist[1][(int) inst.classValue()] -= inst.weight();
      
      // Split criterion initialization
      Split s = getCriterion(currDist);
      ...
      for (int i = 1; i < data.numInstances(); i++) {
        ...
        //System.out.print(inst.value(att) + "(" +inst.classValue() +") - ");
        ...
        if (inst.value(att) > currSplit) {
          ...
          if (currVal > bestVal) {
            ...
            dist = s.getCopyOfContingencyTable();
          }
        }
        currSplit = inst.value(att);

        // Shift over the weight (by the Split class)
        s.shift(1, 0, inst);
      }
    }
  ...
  /**
   * Return the splitting criterion for a given
   * contingency table. The splitting criterion is
   * selected as an option of this classifier.
   * 
   * @param matrix the contingency table
   * 
   * @return the splitting criterion
   */
  protected Split getCriterion(double[][] matrix) {
  	Split s;
  	switch (m_splitCrit){
  	case SPLITCRIT_AMI:
  		Distribution bags = new Distribution(matrix);
  		if (bags.total() <= 700)
  			s = new AMISplit(matrix);
  		else
  			s = new MISplit(matrix);
  		break;
  	case SPLITCRIT_GMI:
  		s = new GrassbergerMISplit(matrix);
  		break;
  	default:
  		s = new MISplit(matrix);
  		break;
  ...
  public static void main(String[] argv) {
    runClassifier(new RandomTreeSplitCriterionSelection(), argv);
  }
   ...
   *             if something goes wrong
   */
  protected int toGraph(StringBuffer text, int num, RandomTreeSplitCriterionSelection parent)
  throws Exception {

    num++;
    if (m_Attribute == -1) {
      text.append("N" + Integer.toHexString(RandomTreeSplitCriterionSelection.this.hashCode())
      ...
      for (int i = 0; i < m_Successors.length; i++) {
        text.append("N"
            + Integer.toHexString(RandomTreeSplitCriterionSelection.this.hashCode())
            + "->" + "N"
            + Integer.toHexString(m_Successors[i].hashCode())
            + " [label=\"");
        if (m_Info.attribute(m_Attribute).isNumeric()) {
